...
\subsection{Notes About the Program}
...
\item[9] We are going to need the {\tt sqrt()} function, which takes one \double\
...
\item[18--20] We haven't seen {\tt scanf()} yet, and we'll talk about it
in detail later.  But it's the reverse of {\tt printf()}: {\tt printf()}
prints data out, and {\tt scanf()} reads data in.  {\tt scanf()}'s first
